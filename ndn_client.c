...
struct ndn {
    int sock;
    size_t outbufindex;
    struct ndn_charbuf *connect_type;   /* 连接状态 text representing connection to ndnd */
    ...
    struct hashtb *keys;    /* 公钥 public keys, by pubid */
...
struct expressed_interest {
    ...
    struct ndn_closure *action;  /* 进来的内容的回调 handler for incoming content */
                    ...
                    struct ndn_closure *src)
{
    ...
    if (src != NULL)
        src->refcount++;
    // 完成，把interest的handler设为传入的action。并没有改变h
    *dstp = src;
 ...
 *
 * 参数
 * name：unix-domain套接字的名字。或者字符串："tcp[4|6][:port]"来指定是使用IPv4还是IPv6.
 *       port是可选的。使用NULL来获得默认设置。
 *       相关环境变量： NDN_LOCAL_TRANSPORT NDN_LOCAL_PORT NDN_LOCAL_SOCKNAME NDN_LOCAL_PORT
 * 返回：连接的文件描述符
 *
 * 过程: 创建socket,连接socket,绑定到h->sock
 *
...
{
    // 所有类型的socket都是sockaddr的强制转换,point to同一个地址
    ...
    if (name != NULL && name[0] != 0) {
        ...
        if (s != NULL && strncasecmp(s, "tcp", 3) == 0) {
          ...
          // 这是最常见的情况,使用 unix domain,不用传进来name参数.
...
#endif
    // 使用获得的sockaddr建立socket。设置了sock
    // sockaddr在开始阶段使用addr指向了,所以被设置了.
                     ...
                     struct ndn_charbuf *interest_template)
{
    ...
    // 通过前缀找到interest。把迭代器e定位到找到的interest上。
    ...
    // 把handler换成传入的参数action
    // 之后interest的action就是我们自定义的了。h不变
...
/**
 * 注册收到特定名字前缀是所执行的callback
 *
 * 如果action是NULL,会删除这个prefix上的所有现存的filter
 * 这样会对共享这些handle的不同应用造成非预期的影响,此时应该使用ndn_set_interest_filter_with_flags
 *
 * namebuf中的内容将会被拷贝
 * handler需要return一个 NDN_UPCALL_RESULT_INTEREST_CONSUMED
 * 来承诺自己已经produce了,或者将要produce相应的Contnet
 *
 * 参数: h, namebuf, action
 *
...
ndn_put(struct ndn *h, const void *p, size_t length)
{
    ...
    if (h->outbuf != NULL && h->outbufindex < h->outbuf->length) {
        ...
        // 把传入参数p的内容加到本ndn client实体的outbuf上。
...
/**
 * 通过h的回调发送message。
 * 不是为常规client准备的，而是在ndnd需要和内部client通讯准备的。
 * 也就是说，ndnd将msg发送给内部client。
 * 参数： msg： ndnb编码过的interest或者data
 *
...
/**
 * 处理调度过的操作。
 * 这个函数不是给常规的ndn client用的，而是给ndnd来运行它内部client的。
 * 返回：下一个需要发生的事距离现在的毫秒数。
 *
...
/**
 * 运行ndn客户端的循环。将timeout设为-1可以当作简单应用的main loop
 *
...
/**
 * 获得一个符合的ContentObject
 * 可以很方便的获得.
 * 会阻塞直到一个符合的ContentObject到来,或者超时
 *
 * 参数
 * h: ndn-handle
 * name: ndnb编码过的名字
 * interest_template: 代表在interest中使用的其他的field.可以是NULL
 * timeout_ms
 * resultbuf: 会保存结果,即ndnb编码过的ContentObject
 *
        ...
        int flags)
{
    ...
    if (h == NULL || h->running) {
        ...
        if (h == NULL)
            return(-1);
        if (orig_h != NULL) { /* Dad, can I borrow the keys? 可以.*/
            saved_keys = h->keys;
            h->keys = orig_h->keys;
        }
    ...
    // 发interest. what if 此interest包含"prefix_reg"的action命令
 ...
 * 参数
 * h => ndn handle
 * resultbuf => 输出的ContentObject就会存在里面
 * name_prefix => 输入的 ndnb 名字前缀
 * params => 参数.sp
 * data => 输入的原始数据内容
 * size => data 的 size
 * 返回
