 ...
 * 参数:
 * h => ndn handle.
 * name => ndnb 的名字前缀. 它被扩展了一个额外的组件,从而能够命名最高版本的内容.
 * versioning_flag => 现阶段必须是 NDN_V_HIGH 或者 NDN_V_HIGHEST, 也可能组合了NDN_V_NESTOK
 *   如果NDN_V_NESTOK没有提供, 而且最后一个组件是一个版本号,函数会立即返回0,
 *   因为它认为一个明确声明的版本被提供了.
 * timeout_ms
 *
