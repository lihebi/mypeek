...
struct ndn_schedule {
    ...
    int heap_n;
    int heap_limit;
    int heap_height;    /* 使用前会被检测合法性 this is validated just before use */
    heapmicros now;     /* 内部始终，记录上次时间 internal micros corresponding to lasttime  */
    struct ndn_timeval lasttime; /* 实际上次检测的时间 actual time when we last checked  */
