...
/**
 * 传入ndnb。将ndnb转换成一个URI，并加入c中。
 * 同时，它也会在ContentObject或Interest中找到名字。
 * 包含单独的0个或者更多个'.'的组件将会被转换。
 * 会额外加上三个'.'，从而不会有关于. .. empty absent的模糊不清。
 * 如果NDN_URI_INCLUDESCHEME不是0，会加上'ndn:'
 * 如果flags & NDN_URI_MIXEDESCAPES不是0，会escape "%" 和 "="
 *
 ...
 * 把s加到c后面。s已经被转义了。这没有任何ndnb相关的工作。
 * 当遇到错误，或者有没有被转义的nul,/,?,#等，会停止。
 *
